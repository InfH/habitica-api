...
HabitRpg.prototype.createTask = function(task, cb) {
  var req = this.buildRequest('POST', '/tasks', task);
  req.end(cb);
};

HabitRpg.prototype.getTask = function(id, cb) {
  var req = this.buildRequest('GET', '/tasks/' + id);
  req.end(cb);
};

HabitRpg.prototype.getTasks = function(cb) {
  var req = this.buildRequest('GET', '/tasks/');
  req.end(cb);
};

HabitRpg.prototype.updateTask = function(id, task, cb) {
  var req = this.buildRequest('PUT', '/tasks/' + id, task);
  req.end(cb);
};

HabitRpg.prototype.deleteTask = function(id, cb) {
  var req = this.buildRequest('DEL', '/tasks/' + id);
  req.end(cb);
};
...
HabitRpg.prototype.updateTaskScore = function(id, direction, cb) {
  ...
  var req = this.buildRequest('POST', '/tasks/' + id +'/' + direction);
  req.end(cb);
};
